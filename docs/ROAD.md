...
* 2026-09-09 — p.2.2 第二块落地（MC 壳注册接线）：runtime.datapack（DatapackRuntime 挂 ServerStarted/Stopping，DatapackRegistrar 扫描装载 + 四类注册）——**recipe 编译为原版 ShapedRecipe → RecipeManager.replaceRecipes 合并**（crafting_shaped），tag/lang 索引进内存 + 确定性 markers，**function 经 TieLogicBundle → engine.tie 0 参调用**；DatapackE2EProbe 开服断言 9 标记全绿；两开服探针经 mustRunAfter 串行（并行抢 session.lock 会锁死 run/world）；E2E 独立 staging 到 build/tmp（避免与 bootProbe 共享 run/datapacks 造成 Windows dll 句柄冲突）。 / p.2.2 block 2 landed: MC-shell registration wiring — recipes into the live RecipeManager, tag/lang indexes, tie functions called; E2E gate green; two boot gates serialized.
