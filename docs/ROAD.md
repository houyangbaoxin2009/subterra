...
* 2026-09-09 — p.2.2 第四块落地（loot 真合并 + worldgen/structure 真注册）：**recipe 规范形往返**（td→ShapedRecipe/SmokingRecipe→规范串，canonical markers 逐字符一致）、**loot_table 真合并进 LOOT_TABLE datapack registry**（1.21.1 无 LootDataManager；`ReloadableServerResources.fullRegistries().get()` RELOADABLE 层，经 `DatapackRegistryInjector` 公开 unfreeze→register→freeze 窗口注入）、**worldgen→CONFIGURED_FEATURE**（configured_feature 最小 schema：simple_block + block）、**structure→STRUCTURE/STRUCTURE_SET**（DesertPyramidStructure 体 + biomes holder）；E2E 16 标记全绿。 / p.2.2 block 4 landed: loot merge into the LOOT_TABLE datapack registry via a public un-freeze window + worldgen/structure real registration; E2E 16 markers green.
* 2026-09-09 — p.2.2 第五块落地（DataPack→td 导出往返）：**RecipeManager → td → 回灌 → 再导出逐字节一致**——engine.datapack `RecipeDatum`（record，read/write 规范 td，count/cooking_time clamp≥1、experience 经 cleanExperience float 往返归一化，镜像 buildShaped/buildSmoking 语义）+ `DatapackExporter` 导出前门；runtime `DatapackRecipeExporter`（holder→datum，pattern 行按格首次出现确定性重建字符 A,B,C…）→ `DatapackRegistrar.runExportRoundTrip`（导出→buildRecipe 回灌→再导出，字节级比对，marker ok）+ 类 javadoc；纯 JVM 探针：导出→DatapackLoader 文件重载→再导出逐字节一致 + 三连稳定 + 导出字段无损（=源 payload 序列化，6 新断言）；E2E 标记 16→18（`export roundtrip toterra:example/smoke ok`）；probeAcceptance 全绿。 / p.2.2 block 5 landed: DataPack→td export round-trip byte-exact (holder→td→buildRecipe→td), engine RecipeDatum schema + mc-shell exporter + registrar round-trip markers + JVM byte-identical probe; E2E 18 markers green.
* 2026-09-09 — p.2.2 第六块落地（导出全种类 + 导出档案 Hub）：**导出器覆盖全部七类**（RECIPE 对象级逐字节往返；TAG/LANG 归一化 TagDatum/LangDatum；LOOT_TABLE/WORLDGEN/STRUCTURE/FUNCTION canonical pass-through、schema 由注册器消费方重建验收）+ **内容级导出档案 DatapackExportArchive**（`type tie<data>` 单文档 `export=[version,entries]`，export∘rehydrate 逐字节恒等，DatapackPack 文件级之外的第二档案形态，p.2.18 engine.export 前哨）；`DatapackRegistrar` 统一 marker `export roundtrip <kind> <id> ok` + `export archive roundtrip ok (entries=10)`；纯 JVM 探针 10 条目文件重载往返逐字节一致 + 档案往返 + 字段无损；E2E 标记 18→27；probeAcceptance 全绿。 / p.2.2 block 6 landed: export covers all seven kinds + content-level export archive (byte-exact rehydrate identity); E2E 27 markers, full gate green.
* 2026-09-09 — p.2.2 第一块落地：**纯 td 直载**（不产出 JSON）——engine.datapack 容器 + 注册表 + tie 逻辑链，DatapackProbe 26 断言全绿；顺带修复共享 Td 解析器 header 误杀 `type = "..."` 数据键与注册表排序丢失两 bug。 / p.2.2 first block landed: pure td direct loading, engine.datapack registry + tie logic chain.
* 2026-09-09 — p.2.2 第二块落地（MC 壳注册接线）：runtime.datapack——**recipe 编译为原版 ShapedRecipe → RecipeManager.replaceRecipes 合并**（crafting_shaped），tag/lang 索引进内存 + 确定性 markers，**function 经 TieLogicBundle → engine.tie 0 参调用**；DatapackE2EProbe 开服断言全绿；两开服探针 mustRunAfter 串行（并行抢 run/world session.lock）；E2E staging 隔离 build/tmp（避开 bootProbe 的 Windows dll 句柄）。 / p.2.2 block 2 landed: MC-shell registration wiring — recipes into RecipeManager, tag/lang indexes, tie functions called; E2E gate green; boot gates serialized.
* 2026-09-09 — p.2.2 第三块落地（打包往返 + 配方类型扩展 + 剩余三类 staging）：**DatapackPack**（目录↔单一 td 文档，base64 content，解包重载注册表/包名/td 载荷逐字节一致）+ **smoking→SmokingRecipe**（buildRecipe 按 type 分派）+ loot/worldgen/structure 三类的 staged 索引与 markers（LootDataManager/RegisterEvent 真注入留第四块，不在本块硬塞）；独立探针 31 断言、E2E 10 标记全绿。 / p.2.2 block 3 landed: DatapackPack round-trip, smoking recipe type, remaining kinds staged with markers; deep vanilla injection deferred to block 4.
