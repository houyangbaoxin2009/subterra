...
* 2026-09-09 — p.2.2 第四块第 1 片落地（规范往返 + 战利品构建）：**recipe 规范形往返**（td→ShapedRecipe/SmokingRecipe→规范串，canonical markers 逐字符一致）、**loot_table→原版 LootTable 对象构建**（td chest schema，pools=1，LootDataManager 真合并留剩余片）、worldgen/structure 仍 staged；E2E 13 断言全绿。 / p.2.2 block 4 slice 1 landed: canonical recipe round-trip markers + LootTable object build; LootDataManager merge and RegisterEvent wiring remain.
