...
| network · holepunch | `HolepunchRuntime`（+ engine `engine.p2p.holepunch` 五件：`PortPredictor` / `HolePunchPlanner` / `PunchPlan` / `PunchAttempt` / `HolePunchSession`） | `runtime.network.holepunch`；engine 核心在 `engine.p2p.holepunch` | E2E 门控 `subterra.probe.holepunch`（缺省 no-op、零 socket、原版握手零接触） | `bootstrap(modContainer)`（仅挂 ServerStarted 门控监听） | `[Subterra holepunch]` | `ServerBootProbe`（`shell active (` / `gate=on loopback ok`）；`HolepunchProbe`（纯 JVM 25 项，含真实 loopback UDP 自打洞） | 已挂接（门控探针壳） |
| ui · title | `TitleRuntime`（+ engine `TitleHintCore`，纯 JDK） | `runtime.ui.title`；engine 在 `engine.ui.title` | E2E 门控 `subterra.probe.title`（缺省 no-op） |  | `bootstrap(modContainer)`（仅 ServerStarted 门控监听） | | `[Subterra title]` | `AsyncE2EProbe`（`gate=on sample ok (`）；`TitleProbe`（纯 JVM 20 项） | 已挂接（门控探针壳） |
| weather | `WeatherRuntime`（+ engine `WeatherCore`，纯 JDK） | `runtime.weather`；engine 在 `engine.weather` | E2E 门控 `subterra.probe.weather`（缺省 no-op） |  | `bootstrap(modContainer)`（仅 ServerStarted 门控监听） | | `[Subterra weather]` | `AsyncE2EProbe`（`gate=on forecast ok (`）；`WeatherProbe`（纯 JVM 14 项） | 已挂接（门控探针壳） |
| render · upscale | `UpscaleRuntime`（+ engine `UpscaleModel`，纯 JDK） | `runtime.render.upscale`；engine 在 `engine.render.upscale` | E2E 门控 `subterra.probe.upscale`（缺省 no-op） |  | `bootstrap(modContainer)`（仅 ServerStarted 门控监听） | | `[Subterra upscale]` | `AsyncE2EProbe`（`gate=on pipeline ok (`）；`UpscaleProbe`（纯 JVM 13 项） | 已挂接（门控探针壳） |
| skin | `SkinRuntime`（+ engine `SkinCore`，纯 JDK） | `runtime.skin`；engine 在 `engine.skin` | E2E 门控 `subterra.probe.skin`（缺省 no-op） |  | `bootstrap(modContainer)`（仅 ServerStarted 门控监听） | | `[Subterra skin]` | `AsyncE2EProbe`（`gate=on resolve ok (`）；`SkinProbe`（纯 JVM 18 项） | 已挂接（门控探针壳） |
...
| worldgen · surface | `SubterraSurfaceRules` / `PaletteRuleSource`（+ engine `SurfacePaletteResolution`，纯 JDK） | `runtime.worldgen.gen`；engine 解析在 `engine.worldgen.pipeline.surfacerules` | 样例安装门控 `subterra.probe.surface`；规则键 `subterra.worldgen.surface_palette` / `subterra.worldgen.surface_palette.<state>`（缺省 `vanilla` = 恒等；boot 属性覆盖层同系） | `bootstrap(modEventBus)`（`RegisterEvent` 向 `Registries.MATERIAL_RULE` 注册 `subterra:surface_palette` rule-source 类型）/ `ServerStarted`（palette 快照 + marker） | `[Subterra surface]` | `ServerBootProbe`（`registered rule-source type … MATERIAL_RULE` / `palette=… remap=… mode=…` / `probe gate=on sample installed: …`）；`SurfaceSeamProbe`（纯 JVM 24 项） | 已挂接（真接缝） |
| worldgen · feature | `SubterraFeatures` / `SubterraFeaturePlan` / `SubterraRuleOreFeature` / `SubterraRuleVeinPlacement` / `SubterraFeatureSupport`（+ engine `FeatureAssembly`，纯 JDK） | `runtime.worldgen.gen`；engine 核心在 `engine.worldgen.feature` | 总开关规则键 `subterra.worldgen.feature.enable`（缺省 false = 恒等）；矿物/植被/护栏键面 `subterra.worldgen.feature.*`（td：`config/subterra/features.td`）；E2E 门控 `subterra.probe.feature` | `bootstrap(modEventBus)`（`RegisterEvent` 注册 `subterra:rule_ore` → `Registries.FEATURE`、`subterra:rule_vein` → `Registries.PLACEMENT_MODIFIER_TYPE`）/ `ServerStarted`（plan marker） | `[Subterra feature]` | `ServerBootProbe`（`registered worldgen types …` / `plan ok (enable=…, mounts=…, guards=…, render=…)`）；`FeatureSeamProbe`（纯 JVM 42 项）/ `FeatureAssemblyProbe`（8 项） | 已挂接（真接缝） |
...
| optim · teleport | `RtpRuntime` / `RtpCommand` / `RtpConfig`（+ engine `RtpPlanner`，纯 JDK） | `runtime.optim.server.teleport.shell`；engine 核心在 `engine.optim.server.teleport` | E2E 门控 `subterra.probe.rtp`；td 规则 `config/subterra/rtp.td`（根键 `rtp`：enabled/min_radius/max_radius/attempts/min_y/max_y/y_offset/command_permission/cooldown_seconds，非法值确定性拒绝） | `bootstrap(modContainer)`（配置预载 + 命令注册 + ServerStarting 种子捕获 / ServerStopped 复位） | `[Subterra rtp]` | `ServerBootProbe`（`shell active (` / `server starting; seed captured`）；`RtpProbe`（纯 JVM 21 项） | 已挂接 |
