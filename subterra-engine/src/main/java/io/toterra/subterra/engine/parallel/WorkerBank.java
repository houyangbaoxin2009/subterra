...
/**
 * One execution lane per bank. Within a bank the queued tasks run strictly
 * serially (FIFO) on a single worker; tasks in different banks run
 ...
 *
 * <p>This is the key-agnostic generalization of the p.2.6
 * {@code engine.worldgen.async.AsyncWorkerBank}: the chunk-specific bank of
 * p.2.6 is lifted verbatim to {@code engine.parallel} with the semantics and
 * implementation skeleton unchanged (single-worker FIFO serial execution,
 * bounded-queue backpressure, exactly-once, awaitIdle/shutdown/join); only the
 * package/class name, the worker thread name and the comments differ. Banks are
 * identified by an opaque {@code int} index and execute arbitrary {@link Runnable}
 * tasks, so any key-space (chunks or otherwise) can be sharded across banks.
 *
 ...
 * a second scheduler layer with its own ordering hazards. As in p.2.6, each bank
 * is therefore backed by one dedicated daemon {@code virtual} worker (JDK 21+,
 * default daemon, so a pure-JVM probe cannot be kept alive). This gives the
 * serial-per-bank + concurrent-across-bank + join contract directly and is
 * trivial to reason about; a later sub-item can still swap in an
 * ExecutorManager-backed implementation behind the same API.
 *
 * <p><b>线程引擎抉择。</b>{@code engine.optim.sched.ExecutorManager} 是共享全局队列、
 * 工作窃取的执行器，没有 per-bank 串行化、任务完成/join 原语、也无序获取接口——
 * 用它复刻"bank 内串行 + 有界 + awaitIdle"会多出一层带自身排序风险的调度。与 p.2.6
 * 一致，这里让每个 bank 独占一个守护 {@code virtual} 工作线程（JDK 21+，默认守护，
 * 纯 JVM 探针不会被拖住）。它直接给出"bank 内串行 + bank 间并发 + join"的契约且易于
 * 推理；后续子项仍可在同一 API 下换成 ExecutorManager 实现。
 *
