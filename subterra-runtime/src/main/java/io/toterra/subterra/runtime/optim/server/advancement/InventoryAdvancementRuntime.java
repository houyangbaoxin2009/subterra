...
package io.toterra.subterra.runtime.optim.server.advancement;

import com.mojang.logging.LogUtils;
import io.toterra.subterra.runtime.optim.server.advancement.config.AdvancementConfig;
import io.toterra.subterra.runtime.optim.server.advancement.index.PlanCompiler;
import io.toterra.subterra.runtime.optim.server.advancement.index.PlayerIndex;
import io.toterra.subterra.runtime.optim.server.advancement.metrics.StatsCollector;
import io.toterra.subterra.runtime.optim.server.advancement.mixin.SimpleCriterionTriggerAccessor;
...
public final class InventoryAdvancementRuntime {
    ...
    public void startupSelfCheck() {
        stats.setEnabled(AdvancementConfig.METRICS_ENABLED.get());
        ...
        try {
            ...
            synchronized (index) {
                PlayerIndex.Selection selection = index.select(inventory, changedStack, registryGeneration.get(), tick,
                        AdvancementConfig.PERIODIC_FULL_SCAN_TICKS.get());
                rawCount = selection.allListeners().size();
                candidateCount = selection.candidates().size();
                double shadowVerifyRate = AdvancementConfig.SHADOW_VERIFY_RATE.get();
    ...
    private void onMismatch(PlayerIndex index, ServerPlayer player, ItemStack stack, PlayerIndex.Selection selection, Verification result) {
        ...
        if (AdvancementConfig.DISABLE_ON_MISMATCH.get()) index.disable();
    ...
    private boolean shouldMaintainIndexes() {
        return AdvancementConfig.ENABLED.get()
    ...
    public void processIndexWarmups(MinecraftServer server) {
        ...
        long budgetNanos = AdvancementConfig.INDEX_WARMUP_BUDGET_MICROS.get() * 1_000L;
    ...
    public OptimizationMode mode() {
        ...
        return override != null ? override : OptimizationMode.parse(AdvancementConfig.MODE.get());
    }

    public String status() {
        PlanCompiler.CacheStats planCache = planCompiler.cacheStats();
        return "mode=" + mode() + ", enabled=" + AdvancementConfig.ENABLED.get() + ", replacementHealthy=" + replacementHealthy
